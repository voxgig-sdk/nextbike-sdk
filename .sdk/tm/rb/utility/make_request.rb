...
module NextbikeUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = NextbikeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NextbikeResponse.new(fetched)
    else
