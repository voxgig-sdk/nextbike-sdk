...
class FtCtrl extends NextbikeControl
{
...
class FtClient
{
    ...
    private NextbikeUtility $utility;

    public function __construct(NextbikeUtility $utility, array $options)
    {
    ...
    public function get_utility(): NextbikeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NextbikeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NextbikeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NextbikeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NextbikeConfig::make_feature($name);
    ...
    private static function buildUrl(NextbikeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NextbikeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NextbikeResponse($fetched);
        }
    ...
    private function fail(NextbikeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NextbikeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
