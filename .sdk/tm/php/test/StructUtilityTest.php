...
// Nextbike SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = NextbikeOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(NextbikeOmni::structfix($now, Struct::undef()))
                        !== json_encode(NextbikeOmni::structfix($val, Struct::undef()))) {
