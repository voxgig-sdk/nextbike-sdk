...
import (
	...
	omni "github.com/voxgig-sdk/nextbike-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NextbikeError); is {
				out := map[string]any{
					"name":    "NextbikeError",
...
func omniCtx(arg any, client *sdk.NextbikeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NextbikeSDK); is {
			client = live
