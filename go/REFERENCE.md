...
| `lat` | `float64` | No | Country center latitude |
| `lng` | `float64` | No | Country center longitude |
...
```go
result, err := client.ReservationStatus(nil).Load(map[string]any{"reservation_id": "reservation_id"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
