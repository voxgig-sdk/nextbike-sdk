...
class TestLiveDataDirect:
    ...
    def test_should_direct_list_live_data(self):
        setup = _live_data_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-live_data", "live" if setup["live"] else "unit")
        if _skip:
