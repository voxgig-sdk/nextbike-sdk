...
class NextbikeTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NextbikeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NextbikeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NextbikeTestRunner.getenv("NEXTBIKE_TEST_LIVE")
        override = NextbikeTestRunner.getenv("NEXTBIKE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NextbikeTestRunner.getenv(key)
        ...
        explain = NextbikeTestRunner.getenv("NEXTBIKE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NEXTBIKE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NextbikeTestRunner._test_control is not None:
            return NextbikeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NextbikeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NextbikeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NextbikeTestRunner.load_test_control()
...
def load_env_local():
    NextbikeTestRunner.load_env_local()


def env_override(m):
    return NextbikeTestRunner.env_override(m)


def entity_data(v):
    return NextbikeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NextbikeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NextbikeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NextbikeTestRunner.load_test_control()


def live_client_options():
    return NextbikeTestRunner.live_client_options()


def live_delay_ms():
    return NextbikeTestRunner.live_delay_ms()
