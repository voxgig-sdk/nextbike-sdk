...
class TestReservationEntity:
    ...
    def test_should_create_instance(self):
        ...
        ent = testsdk.Reservation(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _reservation_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "reservation." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NEXTBIKE_TEST_RESERVATION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        reservation_ref01_ent = client.Reservation(None)
...
def _reservation_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/reservation/ReservationTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "NEXTBIKE_TEST_RESERVATION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("NEXTBIKE_TEST_RESERVATION_ENTID"))
