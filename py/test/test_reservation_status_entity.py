...
class TestReservationStatusEntity:

    def test_should_create_instance(self):
        testsdk = NextbikeSDK.test(None, None)
        ent = testsdk.ReservationStatus(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _reservation_status_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "reservation_status." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NEXTBIKE_TEST_RESERVATION_STATUS_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        reservation_status_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.reservation_status")))
        reservation_status_ref01_data = None
        if len(reservation_status_ref01_data_raw) > 0:
...
def _reservation_status_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/reservation_status/ReservationStatusTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("NEXTBIKE_TEST_RESERVATION_STATUS_ENTID"))
    ...
    if env.get("NEXTBIKE_TEST_LIVE") == "TRUE":
        ...
        client = NextbikeSDK(helpers.to_map(merged_opts))

    _live = env.get("NEXTBIKE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("NEXTBIKE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
