...
class TestLiveDataEntity:
    ...
    def test_should_stream(self):
        ...
        base = NextbikeSDK.test(seed, None)
        seen = list(base.LiveData(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = NextbikeSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.LiveData(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NEXTBIKE_TEST_LIVE_DATA_ENTID JSON to run live")
        client = setup["client"]
...
def _live_data_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("NEXTBIKE_TEST_LIVE_DATA_ENTID"))
