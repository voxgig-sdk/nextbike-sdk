...
The Python SDK for the Nextbike API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.LiveData(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Nextbike(None).load(
    {"id": "test01"}, None
)
