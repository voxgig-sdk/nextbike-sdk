...
describe('ReservationStatusDirect', async () => {
  ...
  test('direct-load-reservation_status', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-reservation_status', setup.live)) return
