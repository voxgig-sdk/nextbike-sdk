...
//     (\`new NextbikeSDK(...)\` / \`NextbikeSDK.test(...)\` become
//     \`NextbikeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"live_data":{"test01":{"id":"test01"}},"public":{"test01":{"id":"test01"}},"reservation":{"test01":{"id":"test01"}},"reservation_status":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NextbikeSDK(...)\` and any
// \`NextbikeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NextbikeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NextbikeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
