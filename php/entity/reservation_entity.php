...
// Nextbike SDK Reservation entity
...
class ReservationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ReservationEntity($this->_client, $opts);
    }

    /**
     * @param Reservation|array $args Reservation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NextbikeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Reservation|array The current Reservation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Reservation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NextbikeHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Reservation fields).
     */
     ...
     *
     * @param ReservationCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ReservationCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Reservation|array The created Reservation as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = NextbikeHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
