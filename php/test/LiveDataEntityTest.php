...
class LiveDataEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = NextbikeSDK::test($seed, null);
        $seen = iterator_to_array($base->LiveData(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = NextbikeConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = NextbikeSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->LiveData(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NEXTBIKE_TEST_LIVE_DATA_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function live_data_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("NEXTBIKE_TEST_LIVE_DATA_ENTID");
