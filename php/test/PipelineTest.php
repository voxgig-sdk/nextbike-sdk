...
{
    private static function utility(): NextbikeUtility
    {
        return new NextbikeUtility();
    }
    ...
    private static function ctx(array $over = []): NextbikeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new NextbikeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new NextbikeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof NextbikeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new NextbikeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = NextbikeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new NextbikeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = NextbikeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new NextbikeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = NextbikeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NextbikeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = NextbikeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new NextbikeResponse([]),
            'result' => new NextbikeResult([]),
        ]);
        [, $err] = NextbikeMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new NextbikeSpec([]),
            'response' => null,
            'result' => new NextbikeResult([]),
        ]);
        [, $err] = NextbikeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new NextbikeSpec(['step' => 's']),
            'response' => new NextbikeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new NextbikeResult([]),
        ]);
        [, $err] = NextbikeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new NextbikeSpec(['step' => 's']),
            'response' => new NextbikeResponse(self::resp(200, ['v' => 1])),
            'result' => new NextbikeResult([]),
        ]);
        [, $err] = NextbikeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new NextbikeSpec(['step' => 's']),
            'response' => new NextbikeResponse(self::resp(200, ['v' => 2])),
            'result' => new NextbikeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        NextbikeMakeResponse::call($ctx);
    ...
    {
        $preset = new NextbikeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new NextbikeSpec([]),
            'response' => new NextbikeResponse([]),
            'result' => new NextbikeResult([]),
        ...
        [$got, $err] = NextbikeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new NextbikeResult([])]);
        [, $err] = NextbikeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new NextbikeSpec([]), 'result' => null]);
        [, $err] = NextbikeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NextbikeSpec(['step' => 's']),
            'result' => new NextbikeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new NextbikeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NextbikeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new NextbikeSpec(['step' => 's']),
            'result' => new NextbikeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new NextbikeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = NextbikeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new NextbikeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new NextbikeSpec([]),
            'result' => new NextbikeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = NextbikeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NextbikeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new NextbikeError('boom', 'boom');
        $utility->fetcher = function (NextbikeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NextbikeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = NextbikeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (NextbikeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = NextbikeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(NextbikeResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (NextbikeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new NextbikeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = NextbikeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new NextbikeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new NextbikeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = NextbikeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = NextbikeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = NextbikeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new NextbikeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, NextbikeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new NextbikeResult(['ok' => false])]);
        $this->expectException(NextbikeError::class);
        NextbikeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new NextbikeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', NextbikeMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        NextbikeMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new NextbikeBaseFeature();
        $b = new NextbikeBaseFeature();
        NextbikeFeatureAdd::call($ctx, $a);
        NextbikeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): NextbikeBaseFeature
    {
        $f = new NextbikeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        NextbikeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): NextbikeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new NextbikeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = NextbikePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new NextbikeResponse(['headers' => null]),
            'result' => new NextbikeResult([]),
        ]);
        NextbikeResultHeaders::call($ctx);
