...
class ReservationStatusEntity
  ...
  def make
    opts = @_entopts.dup
    ReservationStatusEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NextbikeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [ReservationStatus, Hash] the current ReservationStatus data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NextbikeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of ReservationStatus fields)
  def match_get
  ...
  #
  # @param reqmatch [ReservationStatusLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.ReservationStatus.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [ReservationStatus, Hash] the loaded ReservationStatus; raises NextbikeError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NextbikeHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
