...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = NextbikeOmni.make_runner(TEST_JSON_FILE, NextbikeSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : NextbikeOmni.errify(err)
    end
