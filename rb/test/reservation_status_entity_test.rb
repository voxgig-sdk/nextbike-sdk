...
class ReservationStatusEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = reservation_status_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "reservation_status." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NEXTBIKE_TEST_RESERVATION_STATUS_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    reservation_status_ref01_data = nil
    if reservation_status_ref01_data_raw.length > 0
...
def reservation_status_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "reservation_status", "ReservationStatusTestData.json")
  ...
  entid_env_raw = ENV["NEXTBIKE_TEST_RESERVATION_STATUS_ENTID"]
