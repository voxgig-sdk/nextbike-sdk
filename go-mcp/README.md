...
[MCP](https://modelcontextprotocol.io) server exposing the Nextbike SDK as
two agent tools — `nextbike_list` and `nextbike_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nextbike-mcp)
make build

# 2. Provide credentials via the environment
export NEXTBIKE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user nextbike \
  -- /absolute/path/to/nextbike-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./nextbike-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// nextbike_list: first page of records
{ "entity": "live_data" }
{ "entity": "live_data", "query": { } }

// nextbike_load: one record by id
{ "entity": "public", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nextbike-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user nextbike \
     -- "$PWD"/dist/*/nextbike-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `nextbike_list` and `nextbike_load` tools now appear
   in new sessions. Ask the agent to *"list live_data using nextbike"*
   and it calls `nextbike_list` with `{"entity":"live_data"}`.
...
```sh
export NEXTBIKE_APIKEY=sk_live_xxx            # API key
export NEXTBIKE_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `nextbike_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `nextbike_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NEXTBIKE_APIKEY` | API key sent with every request. |
| `NEXTBIKE_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

live_data | public | reservation | reservation_status

### Smoke test via HTTP (raw JSON-RPC)

```sh
./nextbike-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"nextbike_load","arguments":{"entity":"public","query":{"id":1}}}}'
```
