...
type Args struct {
	Entity string         `json:"entity" jsonschema:"live_data | public | reservation | reservation_status"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.NextbikeSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "nextbike_list",
		Description: "List records from Nextbike. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "nextbike_load",
		Description: "Load a single record from Nextbike. " +
...
func runOp(client *sdk.NextbikeSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.NextbikeSDK, name string) (sdk.NextbikeEntity, error) {
	switch strings.ToLower(name) {
	case "live_data":
		return client.LiveData(nil), nil
	case "public":
		return client.Public(nil), nil
	case "reservation":
		return client.Reservation(nil), nil
	case "reservation_status":
		return client.ReservationStatus(nil), nil
