-- Nextbike SDK ReservationStatus entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ReservationStatusEntity = {}
ReservationStatusEntity.__index = ReservationStatusEntity


function ReservationStatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ReservationStatusEntity)
  self._name = "reservation_status"
  self._client = client
...
function ReservationStatusEntity:get_name()
  return self._name
end


function ReservationStatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ReservationStatusEntity.new(self._client, opts)
end
...
function ReservationStatusEntity:mark_deleted()
  self._deleted = true
end


function ReservationStatusEntity:deleted()
  return true == self._deleted
end


function ReservationStatusEntity:data_set(args)
  if args ~= nil then
...
function ReservationStatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ReservationStatusEntity:match_set(args)
  if args ~= nil then
...
function ReservationStatusEntity:match_get()
...
function ReservationStatusEntity:stream(action, args, callopts)
...
function ReservationStatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
