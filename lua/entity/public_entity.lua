-- Nextbike SDK Public entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PublicEntity = {}
PublicEntity.__index = PublicEntity


function PublicEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PublicEntity)
  self._name = "public"
  self._client = client
...
function PublicEntity:get_name()
  return self._name
end


function PublicEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PublicEntity.new(self._client, opts)
end
...
function PublicEntity:mark_deleted()
  self._deleted = true
end


function PublicEntity:deleted()
  return true == self._deleted
end


function PublicEntity:data_set(args)
  if args ~= nil then
...
function PublicEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PublicEntity:match_set(args)
  if args ~= nil then
...
function PublicEntity:match_get()
...
function PublicEntity:stream(action, args, callopts)
...
---@param reqmatch PublicLoadMatch
---@param ctrl? table
---@return Public
---@return string? err
function PublicEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PublicEntity:_run_op(ctx, post_done)
  local utility = self._utility
