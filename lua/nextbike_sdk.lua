...
local NextbikeSDK = {}
NextbikeSDK.__index = NextbikeSDK
...
NextbikeSDK._make_feature = _make_feature


function NextbikeSDK.new(options)
  local self = setmetatable({}, NextbikeSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function NextbikeSDK:options_map()
...
function NextbikeSDK:get_utility()
  return Utility.copy(self._utility)
end


function NextbikeSDK:get_root_ctx()
  return self._rootctx
end


function NextbikeSDK:prepare(fetchargs)
...
function NextbikeSDK:direct(fetchargs)
...
-- by passing it.
function NextbikeSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function NextbikeSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "NextbikeSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function NextbikeSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = NextbikeSDK.new(sdkopts)
  sdk.mode = "test"
