...
The Lua SDK for the Nextbike API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:LiveData(nil):list(nil, nil)
...
local result, err = client:Nextbike(nil):load(
  { id = "test01" }, nil
)
