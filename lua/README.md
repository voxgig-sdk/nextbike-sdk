...
The Lua SDK for the Nextbike API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:LiveData()` — each with the same small set of operations (`list`, `load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
| `lat` | `number` | Country center latitude |
| `lng` | `number` | Country center longitude |
...
```
lua/
├── nextbike_sdk.lua    -- Main SDK module
...
The main module (`nextbike_sdk`) exports the SDK constructor
