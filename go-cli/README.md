...
boru-driven command-line client **and** interactive REPL for the Nextbike
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nextbike-cli)
make build
...
./nextbike-cli list live_data
./nextbike-cli list public

# 5. Override the API base URL for a single call
NEXTBIKE_BASE=https://api.example.com ./nextbike-cli list live_data

# 6. No arguments -> interactive REPL
./nextbike-cli
nextbike> list live_data
nextbike> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nextbike-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nextbike-cli list live_data
   ```
...
Run with no arguments to open a REPL (prompt `nextbike>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `live_data`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nextbike-cli`. |
...
(`./nextbike-cli <expr>`) and interactively (the REPL), and expressions compose the
